...
export const register = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const autoGeneratedName = email.split('@')[0];
    ...
    return res.status(HTTP_STATUS.BAD_REQUEST).json(
      errorResponse("Email already exists or registration data is invalid")
    );
  }
};
...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const token = signToken({ userId: user.id, githubUsername: user.githubUsername || '', role: user.role });
